...
def import_predict_configuration(config_file):
    """
    Read the config file regarding the predicting and import its content
    """
    content = configparser.ConfigParser()
    content.read(config_file)
    config = {}
    ...
    config['sumocfg_file_name'] = content['dir']['sumocfg_file_name']
    config['models_path_name'] = content['dir']['models_path_name']
    tmp = content['dir'].get('model_to_load') 

    tmp2 = list(tmp.split(","))
    for i in range(len(tmp2)):
        tmp2[i] = int(tmp2[i])
    config['model_to_load'] = tmp2
...
def set_test_path(models_path_name, model_n):
    ...
    model_folder_path2 = os.path.join(os.getcwd(), models_path_name, 'model_'+str(1), '')

    if os.path.isdir(model_folder_path):    
        plot_path = os.path.join(model_folder_path, 'test', '')
        os.makedirs(os.path.dirname(plot_path), exist_ok=True)
        plot_path2 = os.path.join(model_folder_path2, 'test', '')
        os.makedirs(os.path.dirname(plot_path2), exist_ok=True)
        return model_folder_path, plot_path, model_folder_path2, plot_path2
    else: 
        sys.exit('The model number specified does not exist in the models folder')


def set_predict_path(models_path_name, model_n, predict_path):
    """
    Returns a model path that identifies the model number provided as argument and a newly created 'test' path
    """
    model_folder_path = []
    for i,v in enumerate(model_n):
        tmp = os.path.join(os.getcwd(), models_path_name, 'model_'+str(v), '')
        model_folder_path.append(tmp)
    #model_folder_path = os.path.join(os.getcwd(), models_path_name, 'model_'+str(model_n), '')

    if os.path.isdir(predict_path):    
        plot_path = os.path.join(predict_path, 'test', '')
        os.makedirs(os.path.dirname(plot_path), exist_ok=True)
